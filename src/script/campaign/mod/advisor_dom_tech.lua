...
local function build(S, B)
	...
	if G.total > 0 then head[#head + 1] = string.format("내 기술표 %d개", G.total) end
	...
	if not G.set then
		L[#L + 1] = ""
		L[#L + 1] = "─ 추천을 만들지 못했습니다"
		L[#L + 1] = string.format("  이 진영의 기술 트리를 동봉한 표에서 찾지 못했습니다(%s).", tostring(G.how))
		L[#L + 1] = "  팩션·서브컬처·컬처 어느 것으로도 일치하는 항목이 없습니다 —"
		L[#L + 1] = "  모드나 신규 DLC 진영일 수 있습니다. 짐작으로 권하지 않겠습니다."
		return L
	end
	...
	if #G.avail > 0 then
		...
		L[#L + 1] = ""
		if G.odd then
			-- 생성기가 표시해 둔 '구조 불확실' 세트(티어 0에도 부모가 걸린 원형 트리).
			-- 후보를 뽑긴 했지만 확신할 수 없다는 걸 숨기지 않는다.
			L[#L + 1] = "─ 고를 수 있어 보이는 기술 (이 진영은 트리 구조가 특이합니다)"
			L[#L + 1] = "  원형으로 짜인 트리라 선행조건 판정이 어긋날 수 있습니다 — 화면에서 확인하세요."
		else
			L[#L + 1] = "─ 지금 고를 수 있는 기술 (선행조건 충족)"
		end
		list_out(G.avail)
	...
	elseif #G.notdone > 0 then
		-- 게임은 연구할 게 있다는데 우리 트리 모델로는 후보가 0개 → 모델이 이 진영과
		-- 안 맞는 것이다(카타이·젠취처럼 원형으로 짜인 트리에서 실제로 그렇다).
		-- 틀린 확신 대신 사실만 말하고, 확실한 것(아직 안 한 기술)만 보여 준다.
		...
		L[#L + 1] = ""
		L[#L + 1] = "─ 고를 수 있는 기술을 찾지 못했습니다."
		L[#L + 1] = "  표의 기술을 전부 보유한 것으로 나옵니다 — 표와 실제가 어긋난 것입니다."
	end
	if G.budget_hit then
		L[#L + 1] = string.format("  ※ 조회 예산(%d회)을 다 써 뒤쪽 기술은 확인하지 못했습니다.", BUDGET)
	end
